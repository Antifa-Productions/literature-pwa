...
/**
 * Decode HTML entities already encoded in the source text:
 * numeric forms (&#39;, &#x27;) plus common named entities.
 * Unknown named entities are passed through untouched.
 */
...
/**
 * Decode the whole source repeatedly until it stops changing, so entities
 * that were escaped MORE than once upstream also normalize. The loop is
 * capped so malformed input can never spin forever.
 */
 ...
 * Structural tags (<pre>, </pre>, <p>, <br>, <div>) are ingestion noise
 * from scraped sources and are stripped, not escaped into view.
 */
function processInlineFormatting(raw) {
    return escapeHtml(raw)
        .replace(/&lt;\/?(?:pre|p|br|div)[^&]*?&gt;/gi, '')
...
async function convertFile(inputPath) {
    ...
    // Single decode pass at ingestion. Any entities already present in the
    // source (&#39;, &lt;, &amp;) are resolved HERE, so the template escapes
    // exactly once and never emits &amp;lt; or &amp;#39; to the browser.
    const decoded = decodeSourceText(raw);

    const book = await buildBookFromText(decoded, inputPath);
